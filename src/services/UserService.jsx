...
const BASE_URL = "https://james-bain-trivia-project-969065f14a5f.herokuapp.com"; // Base URL for API requests
...
const UserService = {
  ...
  checkEmailExists: async (email) => {
    try {
      const response = await axios.post(
        `${BASE_URL}/api/Users/checkEmailExists`,
        { email }
      );
      return response.data;
    } catch (error) {
  ...
  getIdByEmail: async (email) => {
    try {
      const response = await axios.post(`${BASE_URL}/api/Users/getIdbyEmail`, {
        email,
      });
      return response.data;
    } catch (error) {
