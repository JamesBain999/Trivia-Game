...
const BASE_URL = "https://james-bain-trivia-project-969065f14a5f.herokuapp.com"; // Base URL for the backend API
...
const gameStateService = {
  ...
  createGameState: async (data) => {
    try {
      const response = await axios.post(
        `${BASE_URL}/api/gameStates/create`,
        data
      );
      return response.data;
    } catch (error) {
  ...
  getGameStatesbyUserId: async (userId) => {
    try {
      const response = await axios.post(
        `${BASE_URL}/api/gameStates/getGameStatesbyUserId`,
        { userId }
      );
      return response.data;
    } catch (error) {
